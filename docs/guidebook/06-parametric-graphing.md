...
Chapter 4: Cartesian Graphing, Drawing, Formats, and Persistence: a `Param`
graph mode, equation slots holding coordinate pairs, plotting with the same
cancellable redraw, and tracing, tables, and analysis stepped by the
parameter rather than by `X`.
...
`3*COS(T)` and `3*SIN(T)`
evaluates one point of a circle of radius 3 at parameter 0.5.
