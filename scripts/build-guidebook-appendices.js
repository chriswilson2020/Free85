...
for (const group of ledger.groups) {
  ...
  if (group.owner) a += `> Tracked under: \`${esc(group.owner)}\`\n\n`;
...
d += "\n## Open work packages\n\n" +
  "Every non-`equivalent` entry in the Free85 2.0 parity ledger, grouped by " +
  "work package owner. `equivalent` areas are already complete and are " +
  "omitted here (see Appendix A for the full catalog).\n\n";
for (const gap of gaps.gaps) {
  if (gap.status === "equivalent") continue;
  d += `- **${esc(gap.id)}** [${gap.status}, owner \`${esc(gap.owner)}\`, area ${esc(gap.area)}]: ${esc(gap.target)}\n`;
}
