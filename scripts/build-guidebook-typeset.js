...
const EDITION = "For firmware 3.0 · Second Edition";
// Explorations keeps its own constant so the two can diverge again.
const COMPANION_EDITION = "For firmware 3.0 · Second Edition";
...
function colophonHtml(title, { edition = EDITION, note = "" } = {}) {
  return `<section class="colophon"><div class="colophon-push"></div>`
    ...
    + `<p>Free85 3.0 · Typeset from the Markdown sources with pandoc and`
...
function buildCompanion() {
  ...
  if (sources.length !== 12) {
    throw new Error("companion: expected front matter + 9 chapters + solutions "
  ...
  if (chapters.length !== 10) {
    throw new Error(`companion: expected 10 chapter TOC entries, found ${chapters.length}`);
  }
  ...
  if (stats.solutions !== 9) {
    throw new Error("companion: expected 9 solution sections, one per "
