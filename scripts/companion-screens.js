...
// Keys for typing one program-editor line exactly as chapter 6 instructs:
...
// Section 6.5 scores a trial line against the bean data by hand. The slope
...
const CO06_SSD_LINES = ["0->S", "S+(B+M*1-7)^2+(B+M*2-7)^2->S",
...
const co06Score = (intercept, slope) => [
  ...String(intercept).split(""), "STO", "ALPHA", "SIN", "ENTER", 200, "CLEAR", 30,
  ...String(slope).split(""), "STO", "ALPHA", "8", "ENTER", 200, "CLEAR", 30,
  "PRGM", 60, "F1", 120,
  ...CO06_SSD_LINES.flatMap(programLineKeys), 200, "F2", 12000];

const CO06_P1_LINES = ["0->S", "FOR A,1,9", "S+RANDI(0,1)->S", "END", "DISP S", "STOP"];
const CO06_P2_LINES = ["36->N", "0->S", "WHILE N", "S+INT(RANDI(1,6)/6)->S",
  "N-1->N", "END", "DISP S", "STOP"];

// Chapter 7's four Riemann-sum programs: left, right, and midpoint sums with
// four slices, then the midpoint sum with eight.
const CO07_P1_LINES = ["0->S", "FOR A,0,3", "S+EVAL(A/2)->S", "END", "DISP S/2", "STOP"];
const CO07_P2_LINES = ["0->S", "FOR A,1,4", "S+EVAL(A/2)->S", "END", "DISP S/2", "STOP"];
const CO07_P3_LINES = ["0->S", "FOR A,1,4", "S+EVAL((2*A-1)/4)->S", "END", "DISP S/2", "STOP"];
const CO07_P4_LINES = ["0->S", "FOR A,1,8", "S+EVAL((2*A-1)/8)->S", "END", "DISP S/4", "STOP"];

// Chapter 10's differential-equation runs. Seeding stores the initial y into
...
const co10Seed = (value) => [
  ...String(value).split("").map((character) => (character === "-" ? "(-)" : character)),
  "STO", "ALPHA", "0", "ENTER", 120, "CLEAR", 30
];
const CO10_DEQ_MODE = ["GRAPH", 1200, "2ND", "MORE", 90, "MORE", "MORE", 90,
  "F4", 2500, "EXIT", 90];
// The tank of sections 10.1 to 10.5: dy/dx = -.15*Y seeded at 9.
const CO10_TANK = [...co10Seed(9), ...CO10_DEQ_MODE,
  "(-)", ".", "1", "5", "*", "ALPHA", "0", "GRAPH", 6000];
// Section 10.4's Euler walk and section 10.5's improved-Euler pair, whose step
// lives in P3 because eight lines cannot hold driver and method together.
const CO10_P1_LINES = ["9->Y", ".5->H", "7->N", "WHILE N", "Y+H*EVAL(0)->Y",
  "N-1->N", "END", "DISP Y"];
const CO10_P2_LINES = ["9->Y", ".5->H", "7->N", "WHILE N", "CALL 3",
  "N-1->N", "END", "DISP Y"];
const CO10_P3_LINES = ["EVAL(0)->K", "Y+H*K->Y", "Y+H*(EVAL(0)-K)/2->Y", "RETURN"];

// Chapter 5 section 5.2's two solves. The same one-per-cent nudge to a
// right-hand side barely moves a well-conditioned system and sends a
// near-parallel one across the axis, from X 5 to X -5.
const co05Solve = (rows) => ["2ND", "STAT", 60,
  ...rows.flatMap((value) => [
    ...String(value).split("").map((character) => (character === "-" ? "(-)" : character)),
    "ENTER"]),
  "F1", 1500];

// Chapter 4 section 4.4's rational function. (X^2+1)/(X-1) divides out to
...
const CO04_RATIONAL = ["(", "X-VAR", "X^2", "+", "1", ")", "/",
  "(", "X-VAR", "-", "1", ")"];

// Chapter 8 section 8.2's Newton's method. EVAL( and NDER( both read the
...
const co08Newton = (start, steps) => ["X-VAR", "^", "3", "-", "2", "*",
...
const CO10_LOGISTIC = [".", "5", "*", "ALPHA", "0", "*", "(", "1", "-",
  "ALPHA", "0", "/", "1", "0", ")"];
const CO10_GOMPERTZ = [".", "3", "*", "ALPHA", "0", "*", "LN", "1", "0", "/",
  "ALPHA", "0", ")"];

// Chapter 7's two limit specimens, stored into Y1 with [GRAPH]. Both are
// trigonometric and therefore slow, so callers add their own settle frames.
const CO07_SINX = ["SIN", "X-VAR", ")", "/", "X-VAR", "GRAPH"];
const CO07_SINRECIP = ["SIN", "1", "/", "X-VAR", ")", "GRAPH"];

// Chapter 7 spells its calculus commands letter by letter the way the text
// instructs, so the captures type them the same way rather than by name.
function co07Spell(text) {
  const keys = [];
...
const CO10_SYSTEM = ["GRAPH", 100, "2ND", "MORE", "MORE", "MORE", "F4", 600,
  "EXIT", 30, "ALPHA", "0", "GRAPH", 1500, "2ND", "2", 60, "(-)", "X-VAR",
  "GRAPH", 1500, "2ND", "MORE", "MORE", "MORE", "MORE", 40, "F1", 40,
  "F2", "F2", 40, "F3", 40, "+", "+", "+", "+", "+", "+", "+", "+", 40];

// Chapter 8 spells FNINT( letter by letter on the home screen: [ALPHA]
...
// a minus sign is the [(-)] key, as chapter 9 instructs.
...
// Chapter 9 section 9.3 reaches the ill-conditioned COND through the whole
...
const CO09_COND_KEYS = ["2ND", "7", 30, "+", "X-VAR", "+", "X-VAR",
...
// Chapter 11 section 11.1 keeps the pendulum's elliptic integrand in the graph
...
const CO11_PENDULUM = ["2", "*", "2ND", "^", "*", "2ND", "X^2", "2", ".", "5",
  "/", "9", ".", "8", ")", "ENTER", 300, "CLEAR", 30,
  "1", "/", "2ND", "X^2", "1", "-", "ALPHA", "X^2", "*", "SIN", "X-VAR", ")",
  "X^2", ")", "GRAPH", 12000, "EXIT", 120, "CLEAR", 30];
const CO11_RATIO = ["2", "*", ...FNINT_KEYS, "(", "0", ",", "2ND", "^", "/",
  "2", ")", "/", "2ND", "^", "ENTER", 3000, "CLEAR", 30];
const co11Modulus = (halfAngleKeys) => [...halfAngleKeys, "X^2", "STO",
  "ALPHA", "X^2", "ENTER", 60, "CLEAR", 30];

// Chapter 11 section 11.2's two summing programs, and section 11.3's Euler shot.
// Section 11.3's tolerance-driven variant: counts up instead of down and
...
const CO11_TOLERANCE_LINES = ["0->S", "1->N", "WHILE INT(1E4/N^2)",
  "S+1/N^2->S", "N+1->N", "END", "DISP S", "STOP"];
const CO11_P1_LINES = ["0->S", "10->N", "WHILE N", "S+1/N^2->S", "N-1->N",
  "END", "DISP S", "STOP"];
const CO11_P2_LINES = ["0->S", "6->N", "WHILE N", "(1+S)/4->S", "N-1->N",
  "END", "DISP S", "STOP"];
const CO11_P3_LINES = ["10->Y", "20/127->H", "127->N", "WHILE N",
  "Y+H*EVAL(0)->Y", "N-1->N", "END", "DISP Y"];

// Section 11.3's reed bed: dy/dx = -.02*Y^2 seeded at 10 milligrams per litre,
// entered the same way chapter 10 enters the DifEq mode.
const CO11_REED = ["1", "0", "STO", "ALPHA", "0", "ENTER", 120, "CLEAR", 30,
  ...CO10_DEQ_MODE, "(-)", ".", "0", "2", "*", "ALPHA", "0", "X^2",
  "GRAPH", 9000];

// Section 11.4's vector work runs in ANGLE DEG, and leaves the editor between
// sub-explorations because re-entry restores register A, component 1, and the
// first soft-key page in one move.
const CO11_DEGREES = ["2ND", "MORE", 90, "F1", 60, "EXIT", 60];
const co11Components = (values) => values.flatMap((value) => [
  ...String(value).split("").map((character) => (character === "-" ? "(-)" : character)),
  "ENTER"
]);
...
export const SCREEN_CASES = [
  ...
  { name: "co04-cubic-window", keys: ["X-VAR", "^", "3", "-", "4", "*", "X-VAR", "GRAPH", 900] },
  // Chapter 4 section 4.4: the rational function's two branches, with the
  // near-vertical strokes either side of X=1 that are the plotter joining
  // samples a very long way apart.
  { name: "co04-rational", keys: [...CO04_RATIONAL, "GRAPH", 3000] },
  ...
  {
    name: "co04-slant-table",
    keys: [...CO04_RATIONAL, "GRAPH", 3000,
      "2ND", "2", 60, "X-VAR", "+", "1", "GRAPH", 2500, "MORE", 2000]
  },
  // Chapter 4 section 4.2: the slope family X/2, X, 3*X plotted together.
  {
    name: "co04-slope-family",
    keys: ["X-VAR", "/", "2", "GRAPH", 900, "2ND", "2", 30, "X-VAR", "GRAPH", 1800,
      "2ND", "3", 30, "3", "*", "X-VAR", "GRAPH", 2700]
  },
  // Chapter 4 section 4.3: the odd-symmetry test. Y2=f(-x) and Y3=-f(x)
  ...
  // Chapter 4 section 4.4: EXP(X), LN(X), and the mirror line X in the
  // square window; the exponential plots are slow, hence the long waits.
  {
    name: "co04-exp-inverses",
    keys: ["2ND", "LN", "X-VAR", ")", "GRAPH", 8000,
      "2ND", "2", 60, "LN", "X-VAR", ")", "GRAPH", 16000,
      "2ND", "3", 60, "X-VAR", "GRAPH", 20000, "2ND", "-", 26000]
  },
  // Chapter 4 section 4.5: the daylight model 4.3*SIN(PI*X/6) in the
  // standard window.
  {
    name: "co04-daylight-model",
    keys: ["4", ".", "3", "*", "SIN", "2ND", "^", "*", "X-VAR", "/", "6", ")",
      "GRAPH", 8000]
  },
  // Chapter 4 section 4.6: the inverse of f(t)=t^3/10 drawn by the swapped
  // parametric pair x(t)=X^3/10, y(t)=X.
  {
    name: "co04-inverse-pair",
  ...
  // Chapter 5 section 5.1: the tea-blend 3x3 system's unique solution.
  {
    name: "co05-simult-blend",
  ...
  // Chapter 5 section 5.2: timber and labour constraint lines with the 360
  ...
  // Chapter 5 section 5.3: the framer's tableau after SWAP, RADD, RMUL, and
  ...
  // Chapter 5 section 5.4: the NO BOUNDED ROOT notice when the upper bound
  // caps the payment at 300.
  {
    name: "co05-solver-no-root",
    keys: ["2", "4", "STO", "ALPHA", "SIN", "ENTER", 30, "CLEAR",
      "1", "0", "0", "0", "0", "*", "2ND", "LN", "ALPHA", "SIN", "*", "LN",
      "1", ".", "0", "1", ")", ")", "-", "ALPHA", "LOG", "*", "(",
      "2ND", "LN", "ALPHA", "SIN", "*", "LN", "1", ".", "0", "1", ")", ")",
  ...
  // Chapter 5 section 5.5: RREF of the coffee-shop steady-state matrix,
  // stepped to CELL 1 3 where the -2.5 proportion sits.
  {
    name: "co05-markov-steady",
    keys: ["2ND", "7", 30, "+", "X-VAR", "+", "X-VAR",
  ...
  // Chapter 6 section 6.1: the box plot of the keeper's sorted week, bars
  // at 15, 17.5, and 20 between edges standing for 12 and 43.
  {
    name: "co06-box-visitors",
    keys: ["STAT", "+", "+", "+", "+",
  ...
  // Chapter 6 section 6.3: P2's run screen after the section's exact key
  // order (P1 typed and run once first), answering 8 sixes in 36 rolls.
  {
    name: "co06-ssd-poor",
    keys: co06Score(3, 2.5)
  },
  ...
  },
  {
    name: "co06-sim-run",
    keys: ["PRGM", "F1", 30,
      ...CO06_P1_LINES.flatMap(programLineKeys), "F2", 3000,
      "PRGM", 30, "DOWN", "F1", 30,
      ...CO06_P2_LINES.flatMap(programLineKeys), "F2", 12000]
  },
  // Chapter 6 section 6.4: the duckweed pairs on the scatter plot, bending
  // away from any straight line.
  {
    name: "co06-scat-duckweed",
    keys: ["STAT", "+",
  ...
  // Chapter 6 section 6.4: the graph table with the fitted line in Y1 and
  ...
  // Chapter 6 section 6.5: the FCX forecast screen answering the week the
  // pond is full, from the EXPR fit and a stored target of 100.
  {
    name: "co06-forecast-fcx",
    keys: ["STAT", "+",
      "1", "ENTER", "2", "ENTER", "3", "ENTER", "4", "ENTER", "5", "ENTER",
      "ALPHA",
      "6", "ENTER", "1", "2", "ENTER", "2", "4", "ENTER", "4", "8", "ENTER",
  ...
  // Chapter 6 section 6.6: XYLN joining the shuffled tally sheets in entry
  // order, the zig-zag that sorting repairs.
  {
    name: "co06-xyline-zigzag",
    keys: ["STAT", "+", "+", "+", "+",
  ...
  // Chapter 7 section 7.1: SIN(X)/X four ways. Trigonometry is slow to plot,
  ...
  { name: "co07-sinx-standard", keys: [...CO07_SINX, 9000] },
  {
    name: "co07-sinx-zoom-trace",
    keys: [...CO07_SINX, 9000, "+", 5000, "+", 5000, "+", 6000,
      "RIGHT", 400, "RIGHT", 600]
  },
  { name: "co07-sinx-table", keys: [...CO07_SINX, 9000, "MORE", 2000] },
  {
    name: "co07-sinx-table-fine",
    keys: [...CO07_SINX, 9000, "MORE", 2000, "-", 800, "-", 800, "-", 800,
      "-", 1200]
  },
  {
    name: "co07-sinx-eval-error",
    keys: [...CO07_SINX, 9000, "EXIT", 300, "CLEAR", 20,
      ...co07Spell("EVAL(0)"), "ENTER", 600]
  },
  // Chapter 7 section 7.2: SIN(1/X), which has no limit at 0, and
  ...
  { name: "co07-sinrecip-std", keys: [...CO07_SINRECIP, 14000] },
  {
    name: "co07-sinrecip-zoom",
    keys: [...CO07_SINRECIP, 14000, "+", 7000, "+", 7000, "+", 8000]
  },
  {
    name: "co07-sin-cliff",
    keys: [...CO07_SINRECIP, 14000, "EXIT", 300, "CLEAR", 20,
      ...co07Spell("EVAL(.0025)"), "ENTER", 900]
  },
  {
    name: "co07-sin-precision",
    keys: [...CO07_SINRECIP, 14000, "EXIT", 300, "CLEAR", 20,
      ...co07Spell("EVAL(9"), "EE", "(-)", "7", ")", "ENTER", 900]
  },
  {
    name: "co07-squeeze-zoom",
    keys: ["X-VAR", "*", ...CO07_SINRECIP, 14000,
      "2ND", "2", 60, "X-VAR", "GRAPH", 5000,
      "2ND", "3", 60, "(-)", "X-VAR", "GRAPH", 5000,
      "+", 9000, "+", 9000, "+", 10000]
  },
  // Chapter 7 section 7.3: the difference quotient of X^3-2*X at step .01,
  ...
  // Chapter 7 section 7.6: the editor showing P1's line 3, the one worth
  ...
  // [UP] from the blank line 7 land on it.
  {
    ...
    keys: ["X-VAR", "X^2", "+", "1", "GRAPH", 1200, "EXIT", 60, "CLEAR", 30,
      "PRGM", 60, "F1", 120,
      ...CO07_P1_LINES.flatMap((line) => programLineKeys(line)), 200,
      "UP", 60, "UP", 60, "UP", 60, "UP", 250]
  },
  // Chapter 7 section 7.2: NDER(1.5) answering the paper derivative 4.75 of
  // the stored X^3-2*X.
  {
    name: "co07-nder-result",
    keys: ["X-VAR", "^", "3", "-", "2", "*", "X-VAR", "GRAPH", 1200,
      "EXIT", 30, "CLEAR",
      "ALPHA", "9", "ALPHA", "TAN", "ALPHA", "^", "ALPHA", "5",
      "(", "1", ".", "5", ")", "ENTER", 600]
  },
  // Chapter 7 section 7.3: the designed cubic X^3/3-4*X, hill at -2 and
  // valley at 2, in the standard window.
  {
    name: "co07-extrema-cubic",
    keys: ["X-VAR", "^", "3", "/", "3", "-", "4", "*", "X-VAR", "GRAPH", 1200]
  },
  // Chapter 7 section 7.4: the parabola X^2-2*X-3 dipping below the axis
  // between its zeros -1 and 3.
  {
    name: "co07-dip-area",
    keys: ["X-VAR", "X^2", "-", "2", "*", "X-VAR", "-", "3", "GRAPH", 1200]
  },
  // Chapter 7 section 7.5: P4's run screen after the section's exact build
  // order (P1 to P3 typed and run first), the eight-slice midpoint sum
  // answering 4.65625 against FNINT's 4.6666666666667.
  {
    name: "co07-riemann-run",
    keys: ["X-VAR", "X^2", "+", "1", "GRAPH", 1200, "EXIT", 30,
      "PRGM", "F1", 30, ...CO07_P1_LINES.flatMap(programLineKeys), "F2", 6000,
      "PRGM", 30, "DOWN", "F1", 30, ...CO07_P2_LINES.flatMap(programLineKeys), "F2", 6000,
      "PRGM", 30, "DOWN", "F1", 30, ...CO07_P3_LINES.flatMap(programLineKeys), "F2", 6000,
      "PRGM", 30, "DOWN", "F1", 30, ...CO07_P4_LINES.flatMap(programLineKeys), "F2", 9000]
  },
  // Chapter 7 section 7.6: the arch 2-X^2/2 and the line X/2+1 crossing at
  // -2 and 1 in the standard window.
  {
    name: "co07-between-curves",
    keys: ["2", "-", "X-VAR", "X^2", "/", "2", "GRAPH", 1200,
      "2ND", "2", 30, "X-VAR", "/", "2", "+", "1", "GRAPH", 2400]
  },
  // Chapter 8 section 8.1: the designed quartic's root browser opening on
  // ROOT 1, RE 2.7320508075688 (1 plus root 3).
  {
    name: "co08-newton-converged",
    keys: co08Newton(2, 4)
  },
  ...
  // Chapter 8 section 8.8: SIN(X) in the trigonometric window with its
  // degree-3 and degree-5 approximations in the other two slots.
  {
    name: "co08-taylor-slots",
  ...
  // Chapter 9 section 9.4: ANG answering the fourteen-digit right angle for
  // the straightened pair, after the section's subtraction and carry.
  {
    name: "co09-right-angle",
  ...
  // Chapter 9 section 9.6: the combined LU factors of the matrix whose
  ...
  // Chapter 10 section 10.1: the tank's dye concentration decaying across the
  // standard window from the frozen initial condition 9.
  // Section 4.1's window editor, new in 3.0.
  {
    name: "co04-window-editor",
    keys: ["X-VAR", "X^2", "GRAPH", 2500, "2ND", "GRAPH", 60, "MORE", "MORE", 40, "F5", 80]
  },
  // Section 10.8's system setup and phase orbit, new in 3.0.
  {
    name: "co10-deq-system",
  ...
  {
    name: "co10-system-time",
    keys: [...CO10_SYSTEM, "F5", 20000]
  },
  {
    name: "co10-phase-orbit",
    keys: [...CO10_SYSTEM, "MORE", 40, "F5", 20000]
  },
  { name: "co10-tank-decay", keys: [...CO10_TANK] },
  // Chapter 10 section 10.2: the same equation after one [+], the run re-based
  // on the new left window edge and the step halved with it.
  { name: "co10-narrow-window", keys: [...CO10_TANK, "+", 6000] },
  // Chapter 10 section 10.3: the table page holding the 3.5-minute reading of
  // the standard window, 5.290 at X=-6.5, after halving the table step.
  {
    name: "co10-step-table",
    keys: [...CO10_TANK, "MORE", 6000, "-", 5000, "UP", 5000, "UP", 5000,
      "UP", 5000]
  },
  // Chapter 10 section 10.4: P1's run screen, seven Euler steps of 0.5.
  {
    name: "co10-euler-run",
    keys: [...CO10_TANK, "EXIT", 90, "CLEAR", 30, "PRGM", "F1", 60,
      ...CO10_P1_LINES.flatMap(programLineKeys), "F2", 12000]
  },
  // Chapter 10 section 10.5: P2's run screen after the section's build order,
  // the driver calling P3 for each improved-Euler step.
  {
    ...
    keys: [...CO10_TANK, "EXIT", 90, "CLEAR", 30, "PRGM", "F1", 60,
      ...CO10_P1_LINES.flatMap(programLineKeys), "F2", 12000,
      "PRGM", 60, "DOWN", "F1", 60,
      ...CO10_P2_LINES.flatMap(programLineKeys), "EXIT", 120,
      "DOWN", "F1", 60,
      ...CO10_P3_LINES.flatMap(programLineKeys), "EXIT", 120,
      "UP", 60, "F3", 40000]
  },
  // Chapter 10 section 10.6: the equilibrium of .4*(3-Y) approached from below,
  // reached by seeding -6 before the mode's first entry.
  {
    name: "co10-logistic-plot",
    keys: [...co10Seed(1), ...CO10_DEQ_MODE, ...CO10_LOGISTIC, "GRAPH", 14000]
  },
  ...
  {
    name: "co10-logistic-table",
    keys: [...co10Seed(1), ...CO10_DEQ_MODE, ...CO10_LOGISTIC, "GRAPH", 14000,
      "MORE", 4000, "UP", 2500, "UP", 2500]
  },
  { name: "co10-gompertz-plot",
    keys: [...co10Seed(1), ...CO10_DEQ_MODE, ...CO10_GOMPERTZ, "GRAPH", 16000]
  },
  ...
  {
    name: "co10-gompertz-table",
    keys: [...co10Seed(1), ...CO10_DEQ_MODE, ...CO10_GOMPERTZ, "GRAPH", 16000,
      "MORE", 60000]
  },
  // Section 10.7's setup page, which replaced the GDEQ deletion ritual: the
  // method and the initial condition, edited in place.
  {
    name: "co10-deq-setup",
    keys: [...co10Seed(1), ...CO10_DEQ_MODE,
      ".", "4", "*", "(", "3", "-", "ALPHA", "0", ")", "GRAPH", 9000,
      "2ND", "MORE", "MORE", "MORE", "MORE", 60]
  },
  {
    name: "co10-equilibrium",
    keys: [...co10Seed(-6), ...CO10_DEQ_MODE,
      ".", "4", "*", "(", "3", "-", "ALPHA", "0", ")", "GRAPH", 9000]
  },
  // Chapter 11 section 11.1: the integral conservation of energy hands you,
  ...
  // Chapter 11 section 11.3: the same hundred reciprocal squares added upwards
  ...
  {
    name: "co11-tolerance-run",
    keys: ["PRGM", 60, "F1", 120,
      ...CO11_TOLERANCE_LINES.flatMap(programLineKeys), 200, "F2", 90000]
  },
  // Chapter 11 section 11.1: the swing's true period at a 150-degree amplitude,
  // after the section's whole run of modulus stores and ratio probes.
  {
    name: "co11-pendulum-period",
    keys: [...CO11_PENDULUM, ...CO11_RATIO,
      ...co11Modulus(["SIN", "2ND", "^", "/", "3", "6", ")"]), ...CO11_RATIO,
      ...co11Modulus(["SIN", "2ND", "^", "/", "1", "2", ")"]), ...CO11_RATIO,
      ...co11Modulus(["SIN", "2ND", "^", "/", "6", ")"]), ...CO11_RATIO,
      ...co11Modulus(["SIN", "2ND", "^", "/", "4", ")"]), ...CO11_RATIO,
      ...co11Modulus(["SIN", "2ND", "^", "/", "3", ")"]), ...CO11_RATIO,
      ...co11Modulus(["SIN", "5", "*", "2ND", "^", "/", "1", "2", ")"]), ...CO11_RATIO,
      "4", "*", "2ND", "X^2", "2", ".", "5", "/", "9", ".", "8", ")", "*",
      ...FNINT_KEYS, "(", "0", ",", "2ND", "^", "/", "2", ")", "ENTER", 3000]
  },
  // Chapter 11 section 11.2: the reciprocal-squares partial sum at a hundred
  // terms, after the ten- and forty-term runs the section takes first.
  {
    name: "co11-basel-run",
    keys: ["PRGM", "F1", 60, ...CO11_P1_LINES.flatMap(programLineKeys), "F2", 6000,
      "PRGM", 60, "F1", 60, "DOWN", "CLEAR", ...programLineKeys("40->N"), "F2", 24000,
      "PRGM", 60, "F1", 60, "DOWN", "CLEAR", ...programLineKeys("100->N"), "F2", 60000]
  },
  // Chapter 11 section 11.2: the damper series at twelve terms, after the
  // reciprocal-squares program and the six-term run the section takes first.
  {
    name: "co11-damper-run",
    keys: ["PRGM", "F1", 60, ...CO11_P1_LINES.flatMap(programLineKeys), "F2", 6000,
      "PRGM", 60, "DOWN", "F1", 60, ...CO11_P2_LINES.flatMap(programLineKeys), "F2", 6000,
      "PRGM", 60, "F1", 60, "DOWN", "CLEAR", ...programLineKeys("12->N"), "F2", 12000]
  },
  // Chapter 11 section 11.3: the first shot's table page, the outlet reading
  // 1.979 at X=10 against a target of 2.
  { name: "co11-first-shot", keys: [...CO11_REED, "MORE", 6000, "DOWN", 5000] },
  // Chapter 11 section 11.3: the fourth shot's run screen, the seed 10.559
  // landing four hundred-thousandths above the target.
  {
    name: "co11-shot-run",
    keys: [...CO11_REED, "EXIT", 120, "CLEAR", 30,
      "PRGM", "DOWN", "DOWN", "F1", 60, ...CO11_P3_LINES.flatMap(programLineKeys),
      "F2", 60000,
  ...
  // Chapter 11 section 11.4: the three guy tensions adding to a pure downward
  // pull, the third component reading -3600 after the section's carries.
  {
    name: "co11-guy-sum",
    keys: [...CO11_DEGREES,
      "2ND", "8", 60, ...co11Components([9, 120, 0]), 60,
      "MORE", "MORE", 60, "F2", 600, "RIGHT", 30,
      "EXIT", 60, "2ND", "8", 60, ...co11Components([9, 0, -12]), 60,
      "F1", 600, "F2", 600, "RIGHT", 30, "RIGHT", 30,
      "EXIT", 60, "2ND", "8", 60, ...co11Components([9, 0, -12]), 60,
      "ALPHA", 30, ...co11Components([-4.5, 7.7942286341, -12]), 60,
      "ALPHA", 30, "F3", 600, "F5", 900,
      "EXIT", 60, "2ND", "8", 60, ...co11Components([0, 0, 12]), 60,
      "ALPHA", 30, ...co11Components([900, 0, -1200]), 60,
      "ALPHA", 30, "F4", 600, "RIGHT", 30,
      "EXIT", 60, "2ND", "8", 60, ...co11Components([900, 0, -1200]), 60,
      "ALPHA", 30, ...co11Components([-450, 779.42286341, -1200]), 60,
      "ALPHA", 30, "MORE", 30, "F1", 600, "RIGHT", 30, "RIGHT", 30,
      "EXIT", 60, "2ND", "8", 60, ...co11Components([450, 779.42286341, -2400]), 60,
      "ALPHA", 30, ...co11Components([-450, -779.42286341, -1200]), 60,
      "ALPHA", 30, "MORE", 30, "F1", 600, "RIGHT", 30, "RIGHT", 30]
  }
];
