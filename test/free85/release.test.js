...
test("[release.bundle] the stable 2.18 ROM is bound to frozen schemas and reproducibility evidence", async () => {
  ...
  assert.equal(manifest.phase, "15.4");
  assert.equal(manifest.target_release, "2.18.0");
...
test("[release.coverage-performance] release reports retain all parity and timing gates", async () => {
  ...
  assert.equal(performance.phase, "15.4");
